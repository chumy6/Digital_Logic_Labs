...
module FIFO_8_to_3_final_tb();
    ...
    bit queue[$:128];
    function void queue_push_8bits(input [7:0] data_8bits);
        integer j;
    ...
    //test signals status  after reset(full empty half_full)
    initial begin
        in_data=0;
        r_en=0;
        w_en=0;
        ...
        for (i=1;i<5;i=i+1) begin
            ...
            if(out_data!=out_golden_data) begin
        ...
        #25;
        //only in the first loop the data can be written into the fifo
        for (i=1;i<5;i=i+1) begin
            @(negedge clock) w_en=1;in_data=i;
